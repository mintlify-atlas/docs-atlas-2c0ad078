...
<Steps>
  ...
  <Step title="Create your .env file">
    Create a `.env` file at the project root. Docker Compose reads it automatically. See [Configuration](/deployment/configuration) for the full variable reference.

    At minimum, set the port and database variables before continuing:
    ...
    <Warning>
      Change the default database passwords before deploying to any shared or production environment.
    </Warning>
  </Step>
